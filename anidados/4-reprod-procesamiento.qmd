...
datos_2020 <- datos_2020 %>%
  mutate(
    conf_congreso = case_when(conf_congreso %in% c(0, 8) ~ NA, TRUE ~ conf_congreso),
    conf_gob      = case_when(conf_gob %in% c(0, 8) ~ NA, TRUE ~ conf_gob),
    conf_partido  = case_when(conf_partido %in% c(0, 8) ~ NA, TRUE ~ conf_partido)
  ) %>%
  mutate(datos_2020,
                conf_congreso = 4 - conf_congreso,
                conf_gob = 4 - conf_gob,
